...
impl App {
    ...
    fn finish_connect(&mut self, mut dev: Box<dyn JoroDevice>) {
        ...
        // Firmware register: ALWAYS MM-primary. Ground truth 2026-07-07
        // (live BLE test): Fn-primary firmware mode kills the Lock/Copilot
        // Win+X macro composition — those keys emit NOTHING. Meanwhile
        // F-row emission over BLE is plain VK F1-F12 regardless of the
        // register, and the user-facing MM/Fn preference is implemented
        // host-side (remap::build_remap_tables_for_mode). So there is never
        // a reason to put the firmware in Fn mode — pinning MM keeps
        // Lock/Copilot alive in every host-side mode. (Wired returns 0x05
        // NOT_SUPPORTED — treated as silent no-op by the backend.)
        match dev.set_device_mode(false) {
            Ok(()) => {
                eprintln!("joro-daemon: firmware mode pinned MM-primary");
                self.firmware_fn_primary = Some(false);
            }
...
impl ApplicationHandler<UserEvent> for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // Debug log is opt-in (file I/O per keystroke risks the LL hook
        // timeout). Controlled by `hook_debug` in config.toml so a
        // stuck-key incident can be diagnosed without a rebuild.
        remap::set_debug_log(self.config.hook_debug);
