...
**MM/Fn toggle REMOVED entirely (same session, user decision).** Live testing showed toggling was flaky ("takes multiple switches before MM keys work again" — log showed every toggle DID rebuild tables correctly, so the residual flake was UI/hook-swap timing), and with per-key overrides the toggle serves no purpose: MM defaults are always on, and mapping a key to itself (F6 → F6) yields a plain F-key — much easier than the reverse (typing "Backlight+Down" into a remap box). `set_device_mode_pref` handler deleted, all build sites pass `mm_mode=true`, `device_mode` config field kept but ignored (old files still parse), toggle UI replaced with a hint line. User rationale on record: "if every key is individually remappable, the fn/mm toggle is defunct — keep mm default and remap as needed."
...
**Open watch — RESOLVED within the hour:** 0x2A19 IS live — rose 45→46 while plugged (a notify fired for it), fell back 46→45 after unplug. Charging is just glacially slow on the user's port/cable (~1%/h; suggest wall charger / A-to-C).
**2026-07-08 follow-up — overnight charge decoded + lighting cache bug (third time).**
- Overnight: bolt showed, 0x2A19 pegged 100 while plugged (387 identical reads — likely a "charging/charged" display, no gradual climb visible), snapped back to stale 45 at unplug. ~Later the FW pushed a corrective notify 45→93 ON ITS OWN (pre-flip), wired confirmed 93 (`raw 0xed`), BLE then tracked 94/95/91. **Model: 0x2A19 = real gauge with LAZY on-battery sampling (stale windows up to ~1 h after power events, corrected via notify); may peg 100 while charging.** Percent + inference-bolt system is sound; documented in A7.
- **Lighting demo-mode regression after wired flip**: keyboard reverts to default effect on transport switch, but `apply_config`'s `last_applied_lighting` signature cache said "unchanged — skipping writes" → user's static color not restored ("cache was the bug", third occurrence). Fixed: cache cleared at all three disconnect/transport-change sites so every fresh connect rewrites lighting.

**2026-07-08 (later) — "phantom win panels" root-caused: Copilot's BLE emission is BISTABLE; consumer 0x029D decoded + mapped.**
- User's phantom "win status/notifs panel" + "several win shortcuts instead of Greenshot on Copilot": hook_debug (finally enabled live) showed **ZERO VK events** during the misfires — not a stuck modifier at all. The daemon's consumer hook logged `unhandled usage=0x029d raw=[03 9d 02 …]` per press.
- **Copilot over BLE flips between two emissions**: (a) the Win+Shift+F23 VK macro (trigger remap catches it — this worked yesterday), (b) consumer usage 0x029D rid-0x03 (Windows shell handles it directly, no VKs, un-blockable at LL). Flip cause unknown — appeared after the overnight charge/power event. BOTH now mapped to Ctrl+F12 (trigger + consumer-source remap; "Copilot"/"ACViewToggle" added to remap.rs consumer names).
- **0x029D is AMBIGUOUS**: the Fn key over BLE emits the identical rid-0x03 0x029D (Fn+arrow fired Greenshot on first fix). Discriminator shipped: defer the 0x029D dispatch 60 ms, skip when the vendor-report Fn state (fn_detect) says Fn is held. rid-0x02 0x029D remains the dongle Fn-held signal only (was previously treated as Fn unconditionally — that unconditional FN_HELD store on Copilot presses is also gone).
- The stuck-Alt the watchdog auto-released earlier the same day (`remap-watchdog: releasing stuck modifiers [A4]`) was a separate, self-healed event — the watchdog works.
- Settings popover: top-row keys presented by default action ("Remap Volume Down (F6)", From field shows the friendly name, translated back to the functional source on save); tooltips trimmed; hint lines removed per user.

**Charge-indicator verdict:** plugged-vs-unplugged register diff (2 × `diag-battsweep`) showed only `07:86` bytes 7-8 flip — proven UNRELIABLE (same bytes flap while continuously plugged; they echo prior-response residue, e.g. `c7 ca` from `05:80`'s tail). Plus zero GATT notifications on plug/unplug during the probe. **No charge/plugged signal exists over BLE on this FW.** Shipped instead: **charging INFERENCE** — a rising battery level on wireless proves external power → daemon sets `battery_charging`, UI shows the bolt next to the %; cleared on any decrease or disconnect. Also fixed: wired mode-note stuck after USB→BLE switch (renderModeToggle now writes both branches).
